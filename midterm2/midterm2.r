...
set.seed(2718)
w <- sample(rep((1:13),4),13*4,replace=F)
...
# Color the plotting symbol by Species (any 3 colors)
plot(x=1:150, y=iris$Sepal.Length/iris$Petal.Length,col=iris$Species)
...
sd.cache <- sapply(Cache500,sd,na.rm=T)
...
mean.long.cache <- sapply(Cache500, function(x) ifelse(length(x)>=50, mean(x, na.rm=T), NA))
